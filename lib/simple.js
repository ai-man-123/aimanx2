 ...
 let grup = []
  ...
  return participants.filter(a => a.isAdmin)
    ...
    reply(jid, text, quoted, options) {
      return Buffer.isBuffer(text) ? this.sendFile(jid, text, 'file', '', quoted, false, options) : this.sendMessage(jid, text, MessageType.extendedText, { contextInfo: { mentionedJid: this.parseMention(text), forwardingScore: 99999, isForwarded: true, externalAdReply: { title: this.user.name, body: 'Simple Bot WhatsApp With NodeJS', thumbnailUrl: 'https://storage.caliph71.xyz/img/itsuki.jpg', sourceUrl: 'https://github.com/xmell91/mel-bot' } }, quoted, ...options })
    }
  ...
  async toMSG(buffer, type) {
   ...
   return data[type]
    ...
    if (jid.endsWith('g.us')) return this.contacts[jid].name
